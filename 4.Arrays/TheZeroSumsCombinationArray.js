...
function findZeroSumGroups(arr, n){
  ...
  function combination(arr,n){
    ...
    for(let i=0;i<arr.length;i++){
      let smaller = combination(arr.slice(i + 1), n - 1);
      smaller.forEach(c => result.push([arr[i], ...c]));
    }
    return result
  }
  const allComb = combination(arr,n)
  let zeroSum = allComb.filter(h=> h.reduce((acc,cur)=> acc+cur,0) ===0 )
  let unique = Array.from(
    new Set(zeroSum.map(s => JSON.stringify(s.sort((a,b)=> a-b)))))
    .map(s=> JSON.parse(s))
  
    unique = unique.filter(group => new Set(group).size === group.length);
  ...
  if (unique.length === 0) return "No combinations";
...
console.log(findZeroSumGroups(arr,n))